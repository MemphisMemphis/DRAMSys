...
burst_len = 512*32
max_address_bit = 30
trace_file_name = str(burst_len) + '_write_' + str(max_address_bit) + 'bits.stl'
...
with open(trace_file_name, 'w') as trace_file:
    ...
    while (index < max_address/burst_len):
        print('%d: (%d) write 0x%x' % (index, burst_len, addr), file = trace_file);
        addr += burst_len;
        index += 1;
...
with open(random_file_name, 'w') as random_file:
    ...
    for block_index in range(max_address//random_max_address):
        ...
        while (random_index < random_max_address/burst_len):
            ...
            print('%d: (%d) write 0x%x' % (index, burst_len, addr), file = random_file)
            random_index += 1
            index += 1
